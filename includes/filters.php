 ...
 * @return	string	HTML code
 */
function bf_postheaderh5() {
	global $post;
	$postheader = '';

	$postheader .= '<h5 class="entry-title"><a title="' . sprintf( __('Permalink to %s', 'buffet'), get_the_title() ) . '" href="' . get_permalink() . '" rel="bookmark">' . get_the_title() . '</a></h5>';

	echo apply_filters('bf_postheaderh5', $postheader);
}
function bf_postheaderh4() {
	global $post;
	$postheader = '';
	...
	echo apply_filters('bf_postheaderh4', $postheader);
}
/**
...
/**
 * bf_frontbody() - Buffet Framework filter function
 * 
 * Displays news content (summary). By default, it displays the first 50 words of the article.
 * You can override this using the <b>bf_newsbody</b> filter.
 * 
...
if ($attachments) {
	foreach ( $attachments as $attachment ) {
	     $imgarray = wp_get_attachment_image_src($attachment->ID);
		$output .= '<img src="';
		$output .= $imgarray[0];
		$output .= '"/>';
	}
    ...
    echo apply_filters('bf_frontbody', $output); 
}
